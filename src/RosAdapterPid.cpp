...
RosAdapterPid::RosAdapterPid(String cmd_vel, String cmd_pid, String tel_vel) : cmd_vel_(cmd_vel),
                                                                               cmd_pid_(cmd_pid),
                                                                               tel_vel_(tel_vel),
...
void RosAdapterPid::update(ros::Time &current_time)
{
    if (wheel_ != 0)
    {
        ...
        HardwareController *controller_ = wheel_->getHardwareController();
        if (controller_ != 0)
            tel_msg_.demanded_duty = controller_->duty();
